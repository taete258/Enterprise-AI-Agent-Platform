...
export default function AuditPage() {
  ...
  return (
    <section>
      <PageHeader title="Audit log" subtitle="ประวัติการดำเนินการ 200 รายการล่าสุด" />
          ...
          <table className="w-full text-[13px]">
            <thead>
              <tr className="bg-muted border-b border-border">
                {["เวลา", "User", "Action", "Resource", "IP", "Detail"].map((h) => (
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} className="border-b border-border hover:bg-accent/40">
                  <td className="px-3 py-2.5 whitespace-nowrap text-muted-foreground text-[10.5px] font-mono">{new Date(r.created_at).toLocaleString("th-TH")}</td>
              ...
              {rows.length === 0 && (
                <tr><td colSpan={6} className="text-center text-muted-foreground py-6">ยังไม่มีกิจกรรม</td></tr>
              )}
            </tbody>
          </table>
        </Card>
