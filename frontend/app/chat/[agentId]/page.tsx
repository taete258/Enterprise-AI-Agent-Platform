...
import { agents, sessions, llm, auth } from "@/lib/api";
...
export default function ChatPage() {
  ...
  useEffect(() => { 
    agents.get(id).then(setAgent).catch(() => {}); 
  }, [id]);

  // Fetch available models
  useEffect(() => {
    llm.models().then(setModels).catch(() => {});
  }, []);
  ...
  // Fetch all agents to map agent names in history
  useEffect(() => {
  ...
  // Fetch/refresh sessions list whenever sessionId changes
  useEffect(() => {
    sessions.list().then(setSessionsList).catch(() => {});
  }, [sessionId]);

  // Handle auto-loading messages or redirecting to existing sessions
  useEffect(() => {
    if (sessionIdQuery) {
      ...
      if (!isNew && sessionsList.length > 0) {
        ...
        if (existingSession) {
          router.replace(`/chat/${id}?session_id=${existingSession.id}`);
          return;
        }
      }
  ...
  async function send() {
    ...
    try {
      ...
      if (!sessionId) {
        router.replace(`/chat/${id}?session_id=${r.session_id}`);
      }
    } catch (e: any) {
      ...
      <div className="flex h-full min-w-0 overflow-hidden">
        ...
        <aside className="w-64 border-r border-border bg-sidebar/35 flex flex-col shrink-0 min-w-0">
          <div className="p-4 border-b border-border flex items-center justify-between shrink-0">
            <h2 className="font-serif text-[15px] font-semibold text-sidebar-foreground tracking-tight">Recent chats</h2>
            <Button
              variant="outline"
              size="sm"
              className="text-[11px] h-7 px-2.5"
              onClick={() => router.push(`/chat/${id}?new=true`)}
            >
              + New Chat
            </Button>
          </div>
          <div className="flex-1 overflow-y-auto p-2 space-y-1">
            {sessionsList.length === 0 ? (
              <p className="text-[11.5px] text-muted-foreground p-3 text-center">ยังไม่มีประวัติการแชท</p>
            ) : (
              sessionsList.map((s) => {
                ...
                return (
                  <button
                    key={s.id}
                    onClick={() => router.push(`/chat/${s.agent_id}?session_id=${s.id}`)}
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-background">
          <Topbar
            ...
            right={<span className="font-mono text-[10.5px] text-muted-foreground">{sessionId ? `session #${sessionId}` : "new chat"}</span>}
          />
              ...
              <div className="max-w-3xl mx-auto px-6 py-3.5">
                <Card className="p-1.5 flex items-end gap-1">
                  <textarea
                    ...
                    placeholder="ส่งข้อความถึง Agent…"
                    value={input}
                ...
                <div className="flex items-center justify-between mt-2 text-[10.5px] text-muted-foreground px-1">
                  <div className="flex items-center gap-2">
                    <span><kbd>Enter</kbd> ส่ง · <kbd>Shift</kbd>+<kbd>Enter</kbd> ขึ้นบรรทัด</span>
                    {agent && (
                      <>
...
function Welcome({ name }: { name?: string }) {
  return (
    <div className="text-center pt-10">
      <h2 className="font-serif text-3xl text-foreground tracking-tight">Hi, ฉันคือ {name || "Agent"}</h2>
      <p className="text-[14px] text-muted-foreground mt-2">เริ่มต้นด้วยคำถามใดก็ได้ — มาดูกันว่าจะช่วยอะไรคุณได้บ้าง</p>
    </div>
  );
}
  ...
  return (
    <div className="p-5 space-y-6">
      <div>
        ...
        <p className="text-[12px] text-muted-foreground mt-1">{agent.description || "ไม่มีคำอธิบาย"}</p>
      </div>
      ...
      <div>
        <div className="section-h mb-2">System prompt</div>
      ...
      <div className="space-y-2.5">
        <div className="section-h">Session stats</div>
        <Row k="Messages" v={String(msgCount)} />
        <Row k="Tokens in" v={totalIn.toLocaleString()} />
        <Row k="Tokens out" v={totalOut.toLocaleString()} />
      </div>
    </div>
  );
}
