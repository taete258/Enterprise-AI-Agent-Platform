...
export default function NewAgentPage() {
  ...
  const t = useTranslations("NewAgentPage");
  ...
  useEffect(() => {
    ...
    admin.listTools().then(setAllTools).catch(() => {});
  }, []);
  ...
  async function submit() {
    ...
    try {
      ...
      const toolPayload = Object.entries(selectedTools)
      ...
      if (toolPayload.length > 0) {
        await agents.updateTools(a.id, toolPayload);
      }
  ...
  return (
    <AppShell>
      <Topbar title={t("title")} subtitle={STEPS[step - 1]}
              right={<Button variant="ghost" size="sm" onClick={() => router.back()}>{t("cancel") ?? "Cancel"}</Button>} />
            ...
            {step === 1 && (
              <div className="space-y-4">
                <div className="space-y-1.5">
                  <Label>{t("agentName")} <span className="text-primary">*</span></Label>
                  <Input placeholder={t("namePlaceholder")} value={form.name}
                         onChange={(e) => set("name", e.target.value)} />
                </div>
                <div className="space-y-1.5">
                  <Label>{t("description")}</Label>
                  <Textarea placeholder={t("descPlaceholder")}
            ...
            {step === 2 && (
              <div className="space-y-5">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <Label>{t("model")} <span className="text-primary">*</span></Label>
                    ...
                    {models.length === 0 && (
                      <Alert variant="warning"><AlertDescription>{t("noModels")}</AlertDescription></Alert>
                    )}
                  </div>
                  <div className="space-y-1.5">
                ...
                <div>
                  ...
                  <div className="flex justify-between text-[10.5px] text-muted-foreground mt-1">
                    <span>{t("precise")}</span><span>{t("balanced")}</span><span>{t("creative")}</span>
                  </div>
                </div>
              </div>
            )}
                        ...
                        <label className="flex items-start gap-3 cursor-pointer">
                          ...
                          <div>
                            <div className="text-[13px] font-medium text-foreground flex items-center gap-2">
                              {tool.name}
                              <code className="text-[10px] font-mono bg-muted text-muted-foreground px-1 py-0.5 rounded">{tool.key}</code>
                            </div>
                            <div className="text-[12px] text-muted-foreground mt-0.5 leading-relaxed">{tool.description}</div>
                          </div>
                        </label>
